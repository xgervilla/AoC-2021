...
def allFlashes(octopusmap):
	...
	allZeros = True
	#dejamos los que han flasheado a 0 y los contamos -> si TODOS estan a 0, hemos llegado al flash comun 
	for row in range(rows):
		for col in range(cols):
			if octopusmap[row][col] == -1:
...
if __name__ == '__main__':
	...
	steps = 1
	#mientras haya puplos que no flashean (simultaneamente), calculamos un nuevo paso
	while not allFlashes(octopusmap):
		steps+=1
